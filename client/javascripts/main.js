...
//var socket = io.connect('https://socket-io-message-viewer-unexempt-eardrop.mybluemix.net');
...
$(document).ready(function() {
  ...
  $.getJSON('/applicationId', function(response, statusText, jqXHR) {
      if(jqXHR.status == 200) {
        $('#application-id').show();
        $('#application-id-value').html(response.id);
      }
  });
});